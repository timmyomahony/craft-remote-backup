(function($) {
  Craft.RemoteBackupUtility = Garnish.Base.extend({
    init: function(id) {
      ...
      this.$trigger = $("input.submit", this.$element);
      this.$status = $(".utility-status", this.$element);

      this.listActionUrl = this.$table.find("input[name='action']").val();
      this.createActionUrl = this.$form.find('input[name="action"]').val();
      // this.addListener(this.$element, "submit", "onSubmit");
    ...
    updateTable: function(backups) {
      if (backups.length > 0) {
        this.hideTableNoResults();
